...
export class Analyzer {
  ...
  /**Constructor */
  constructor(login, {shell, account = "bypass", rest = null, uid = Math.random(), skipped = [], authoring = [], timeout = NaN}) {
    this.login = login
    this.shell = shell
  ...
  async run(runner) {
    return new Promise(async solve => {
      if (Number.isFinite(this.timeout)) {
        this.debug(`timeout set to ${this.timeout}m`)
        setTimeout(() => {
          this.results.partial = true
          this.debug(`reached maximum execution time of ${this.timeout}m for analysis`)
          solve(this.results)
  ...
  async filter(path) {
    ...
    try {
      ...
      this.debug(`an error occured during filtering of commits authored by ${this.login} in ${path} (${error})`)
      return commits
    }
  }
